 ...
 * 
 */
public class CheckInfoAnswer implements IServerAction {
	...
	public String executeServerAction(final HttpServletRequest request,
		...
		for(Map<String,String> mParams:lstData){
            String content=mParams.get("ids");
            if(content!=null){
            	session.setAttribute("ids", content);
		...
		for(int i = 0;i  < str.length;i++ ) {
			System.out.println("id" + i + ":" + str[i]);
			String id = str[i];
			PreparedStatement pstMachine = null;
			ResultSet rsMachine = null;		
			String sqlMachine = "select m.MachineCode,m.UnitId,u.UnitName from MachineInfo m,UnitInfo u where m.id = '" + id + "' and u.id = UnitId;"; 
			String machineCode = "";
			String unitId = "";
			String unitName = "";
