  ...
  <head>
	...
	$(document).ready(function(){	 	
		...
		function myLoad(pageNo,year,month) {	
			$.ajax({
				url: "admin/evaluate/machineEvaluate_getMachineEventCount.action?year=" + year + "&month=" + month + "&pageNo" + pageNo,
				method:'get',
				dataType:'json',
				contentType:"application/json; charset=utf-8",
				cache:false,
				success:function(machineJSONArray){
					//alert(unitJSONArray.length);
					if(machineJSONArray.length != 0){
						$("tbody#eventinfo").empty();
						$.each(machineJSONArray,function(index,value){
							var machineName = value['machineName'];
							var num = index + 1;
							var count = value['count'];
							var machineCount = value['machineCount'];
							...
							$("tbody#eventinfo").append(
								...
								+ "<td><a  id=\"link" + num + "\" href=\"javascript:void(0);\" data-toggle=\"collapse\" data-target=\"#graphic\"><small>" + machineName + "</small></a></td>"
								+ "<td><small>" + machineCount + "</small></td>"
							...
							$(aid).on("click",function(){
								//$(aid).text();
								var param = encodeURI(encodeURI(machineName)); 
								//alert(machineName);
								$.ajax({
									url: "admin/evaluate/machineEvaluate_getCountByMachine.action?year=" + year + "&month=" + month + "&machineParam=" + param,
									method:'get',
									dataType:'json',
									contentType:"application/json; charset=utf-8",
									cache:false,
									success:function(count1JSONArray){
										if(count1JSONArray.length > 0) {
											var options = myChart.getOption();
											$.each(count1JSONArray,function(index,value){
...
<table class="table table-bordered table-hover table-condensed">
    ...
    <tr id="tr">
        ...
        <th rowspan="2"><small>设备名称</small></th>
        <th rowspan="2"><small>数量</small></th>
