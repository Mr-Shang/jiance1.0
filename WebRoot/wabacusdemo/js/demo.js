...
function testTypePromptCallBack(textObj,colvaluesArr)
{
	...
	//执行此报表的搜索操作
	var strs = textObj.id.split("_");
	var pageid = strs[0];
	var reportid = strs[2];
	//wx_alert();
	searchReportData(pageid,reportid);
	/**
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function GetReportData(pageid,reportid,reporttype,columns,conditions)
{
	var datasObj=getEditableReportColValues(pageid,reportid,reporttype,columns,conditions);
	if(datasObj==null)
	{
	...
	}else if(isArray(datasObj))
	{
		...
		for(var i=0;i<datasObj.length;i=i+1)
		{
			...
			for(var key in dataObjTmp)
			{
				if(dataObjTmp[key].name == "Count" && dataObjTmp[key].value != "0"){
					count1 = count1 + 1;
				}
				if(dataObjTmp[key].name == "UnitId"){
					unitid = dataObjTmp[key].value;
				}
			}
		}
		str=unitid+'+'+count1;
		invokeServerActionForReportData('ErrorInfo','report2','com.ccu.wabacus.test.Test2',null,{str:str});
	}else
	{
		var count1=0;
		var unitid='';
		...
		for(var key in datasObj)
		{
			if(datasObj[key].name == "Count" && datasObj[key].value != "0"){
				count1 = count1 + 1;
			}
			if(datasObj[key].name == "UnitId"){
				unitid = datasObj[key].value;
			}
		}
		str=unitid+'+'+count1;
		invokeServerActionForReportData('ErrorInfo','report2','com.ccu.wabacus.test.Test2',null,{str:str});
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function testCallbackMethod(datasObj)
{
	if(datasObj.returnValue!=null&&datasObj.returnValue!='')
	{
		var str = datasObj.returnValue;
		var no = str.split('*');
        var unitid = no[0];
        var result = no[1];
        alert('抽检率为:"'+result+'"');
        wx_winpage('ShowReport.wx?PAGEID=ErrorInfo&unitid"'+unitid+'"',{width:800,height:400,title:'完好率'});
        //wx_confirm('抽检率为:"'+result+'"点击"确认"计算完好率！','测试confirm',300,200,okToGo,cancelBack);
	}
}
/*function okToGo(){
	wx_winpage('ShowReport.wx?PAGEID=ErrorInfo&unitid"+unitid+"',{width:800,height:400,title:'完好率'});
 ...
 * 显示详细信息页面
 */
function toSummary(url,param) {
//	alert("跳转目标路径：" + url);
//	alert("传过来的参数：" + id);
	var targetUrl = url + "&param=" + param;//拼接目标路径
//	alert(targetUrl);
	var p1 = document.getElementById("WX_CONTENT_monitoring_guid_row");
	p1.innerHTML = "";
	p1.innerHTML = "<iframe src='"+targetUrl+"' name='ifrm' id='ifrm' width='600' height='200'></iframe>";
}
/**
 * 获取选中行
 */
function getSelectedValues(pageid,reportid) {
	var allSelectedTrObjsArr=getListReportSelectedTrObjs(pageid,reportid);
	if(allSelectedTrObjsArr==null||allSelectedTrObjsArr.length==0) return null;
	var str='';
	for(var i=0;i<allSelectedTrObjsArr.length;i++)
	{
		var tdChilds=allSelectedTrObjsArr[i].getElementsByTagName('TD');
		for(var j=0;j<tdChilds.length;j++)
		{
			var name=tdChilds[j].getAttribute('value_name');//获取当前列对应的<col/>的列名
			var value=tdChilds[j].getAttribute('value');//获取选中行的当前列的数据
...
function gotoServerAction(pageid,reportid,serverClassName,afterCallbackMethod,onErrorMethod)
{
	var datas =  getSelectedValues(pageid,reportid);
	if(confirm("是否进行查岗？")) {
		...
		invokeServerAction(serverClassName,datas,afterCallbackMethod,onErrorMethod);
		wx_winpage('http://localhost:8080/jiance1.0/ShowReport.wx?PAGEID=CheckType');
	}
}
/**
 * 获取查岗类型（单选）
 */
function getType(pageid,reportid) {
	var allSelectedTrObjsArr=getListReportSelectedTrObjs(pageid,reportid);
	if(allSelectedTrObjsArr==null||allSelectedTrObjsArr.length==0) {
		...
		var strt='';
		for(var i=0;i<allSelectedTrObjsArr.length;i++){
			var type=allSelectedTrObjsArr[i].getElementsByTagName('TD');
			for(var j=0;j<type.length;j++){
				var name=type[j].getAttribute('value_name');//获取当前列对应的<col/>的列名
				var value=type[j].getAttribute('value');//获取选中行的当前列的数据
				if(name&&name!=''&&name=='Type')
					strt=value;
			}
		}
		invokeServerAction('com.ccu.wabacus.checkInfo.CheckInfoAnswer',{type:strt},closeDialog);
		wx_success("查岗成功！");
		return strt;
		}
}
...
function getTypeAll(pageid,reportid) {
	var allSelectedTrObjsArr=getListReportSelectedTrObjs(pageid,reportid);
	if(allSelectedTrObjsArr==null||allSelectedTrObjsArr.length==0) {
		wx_warn('请选择查岗类型!');
		return null;
		...
		invokeServerAction('com.ccu.wabacus.checkInfo.CheckAllAnswer',{type:strt},closeDialog);
		wx_success("查岗成功！");
		return strt;
		}
}
...
function deleteSelectValues(pageid,reportid) 
{
	var allSelectedTrObjsArr=getListReportSelectedTrObjs(pageid,reportid);
	if(allSelectedTrObjsArr==null||allSelectedTrObjsArr.length==0)
	{
	...
	deleteEditableListReportRows(pageid,reportid,allSelectedTrObjsArr);
	
	alert("hhh");
}

function beforeDeleteMethod(){
	wx_confirm('确认删除报表数据');
}

function beforeAddMethod(){
	wx_confirm('确认添加报表数据');
}

function beforeUpdateMethod(){
	wx_confirm('确认修改报表数据');
}

function showEventInfo()
{
	refreshComponentDisplay('fireMonitoring','showEventInfo',false);
}

function timerRefresh(pageid,componentid)
{
	 
	setInterval('showEventInfo()','5000');
}
